...
`Lexer.navigator()` builds the token navigator from `navigator.luau`, which wraps a scan in a coroutine and caches every token it pulls. `Next` advances through the cache or resumes the scan, and `Peek` reads ahead of or behind the current position without moving it, using negative offsets for already read tokens. `SetSource` and `HotswapSource` both restart the scan from the beginning, because the scanner carries context across tokens and restarting mid-source could misclassify what follows.
