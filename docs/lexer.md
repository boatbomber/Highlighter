...
`Lexer.navigator()` builds the token navigator from `navigator.luau`, which wraps a scan in a coroutine and caches every token it pulls. `Next` advances through the cache or resumes the scan, and `Peek` reads ahead of or behind the current position without moving it, using negative offsets for already read tokens. `SetSource` clears the cache and restarts the scan from the beginning. `SeekToByte` moves the position to the token containing a given byte and returns how many of that token's bytes precede it, which lets a consumer resume reading partway through the stream by trimming its first read.

`HotswapSource` replaces the source while keeping every cached token the edit provably left untouched. Like `SetSource`, it resets the read position, so the next `Next` returns the first token of the new source even when that token came from the cache. The scanner reports a restart snapshot alongside each token that ends somewhere safe to resume, meaning no type context is open, no interpolation hole is pending, and the alias and typeof machinery is idle. Each snapshot carries the bracket depth at that point and a watermark recording the furthest byte the scan had read by then, lookahead included. On a hotswap the navigator measures the shared byte prefix of the old and new sources and walks back to the latest snapshot whose watermark sits inside it. Everything the scanner read to produce those tokens and reach that state is identical in both sources, so the kept tokens and the resumed scan match a full scan of the new source exactly. The resumed scan seeds its bracket depth from the snapshot and its previous token texts from the cache, which keeps the library member heuristic working across the seam. When no snapshot qualifies, the navigator falls back to a full rescan. `HotswapSource` returns how many leading bytes of the new source its kept tokens cover, which is the prefix whose tokenization provably matches the previous source.

Label population is the built-in consumer of this machinery. Every highlighted TextObject keeps its own navigator, and each text change hotswaps it to the new source, then rebuilds rich text only from the first line the edit could have changed, found by walking the newlines inside the reused byte count and seeking to that line's first byte. Typing near the end of a source therefore pays for rescanning and rebuilding just the tail lines, and a force update with unchanged text, like a theme or customLang change, rescans nothing at all.

What populate requires of a navigator is the public `TokenNavigator` contract in `types.luau`: `SetSource`, `HotswapSource` returning the reused byte count, `SeekToByte` returning the head trim, `Next`, and `Destroy`. A custom lexer's factory whose navigator misses any of those methods is ignored, and that lexer renders through the plain scanning path instead.
